...
class VLNCEDatasetV1(Dataset):
    ...
    config: Optional[Config] = None,
        ...
        # 获取要加载的 episode 数量
        episodes_to_load = (
            config.EPISODES_TO_LOAD 
            if config and config.EPISODES_TO_LOAD 
            else len(deserialized["episodes"])
        )
        # print(config)
        target_episode_ids = (
            config.TARGET_EPISODE_IDS 
            if config and config.TARGET_EPISODE_IDS 
            else None
        )

        print(f"Loading {episodes_to_load} episodes...")
        print("************************************************************************")
        print("target_episode_ids", target_episode_ids ) 
        # 检查 config 是否指定了 TARGET_EPISODE_IDS（假设 config 有这个字段）
        # target_episode_ids = getattr(config, "TARGET_EPISODE_IDS", None)
        target_episode_ids = [target_episode_ids]  # 修复类型不一致
        print("target_episode_ids", target_episode_ids )
        ...
        for episode in deserialized["episodes"][:episodes_to_load]:
            ...
            if target_episode_ids is None or episode.episode_id in target_episode_ids:
                self.episodes.append(episode)

        self.episodes.sort(key=lambda x: x.episode_id)
        print(f"✅ Final number of loaded episodes: {len(self.episodes)}")
