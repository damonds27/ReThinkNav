...
class llmClient:
    def __init__(self, model_type = '', api_key=None, base_url=None):
        ...
        elif model_type == "qwen3-max-preview":
            self.model = model_type
            self.client = OpenAI(
                api_key=api_key,
...
class spatialClient:
    ...
    def observe_view(self, logger, current_step, direction_idx, direction_image):
        ...
        spatial_scene_description_prompt = f"What objects are in the image, and how far are these objects from the camera, calculate the result in meter."
        ...
        direction_desc = DIRECTION_DESCRIPTIONS[int(direction_idx)]
        observe_result = (
            f"Direction {direction_idx} Direction Viewpoint ID: {direction_idx} "
            f"in Step ID: {current_step} Elevation: Eye Level {view_observation}"
            f"Direction Description: {direction_desc}"
        )
