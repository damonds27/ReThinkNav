...
class BaseVLNCETrainerLLM(BaseILTrainer):
    ...
    def _initialize_policy(
        ...
        # self.waypoint_predictor.load_state_dict(
        #     torch.load(
        #         './waypoint_prediction/checkpoints/check_cwp_bestdist_hfov90',
        #         map_location = torch.device('cpu'),
        #     )['predictor']['state_dict']
        # )
    ...
    def generate_input_without_delete(self, observations, current_step, current_episodes):
        ...
        for key in observations.keys():
            image_path = f"./image_show/{current_episodes}/{current_step}/"
            if 'rgb' in key:
    ...
    def generate_input(self, observations, current_step, current_episodes):
        instruction = observations['instruction']['text']
        image_dict = {} 
        rgb_image_dict = {}
        depth_image_dict = {}
        rgb_index = 0
        depth_index = 0
        episode_dir = f"./image_show/{current_episodes}"
        if current_step == 0 and os.path.exists(episode_dir):
            shutil.rmtree(episode_dir)
            os.makedirs(episode_dir)

        for key in observations.keys():
            image_path = f"{episode_dir}/{current_step}/"
        ...
        for index in rgb_image_dict:
            image_dict[index] = {
                'rgb': rgb_image_dict[index],
                'depth': depth_image_dict.get(index)
            }

        return instruction, image_dict


    def parse_done_or_not(self, response_text):
        lines = response_text.strip().splitlines()
        start_check = False
        start_keyword  = "Done_or_Not"
    ...
    def _eval_llm(
        ...
        if config.EVAL.SAVE_RESULTS:
            ...
            if os.path.exists(fname):
                print(f"Overwriting existing evaluation results at: {fname}") 
        envs = construct_envs(
        ...
        envs.number_of_episodes = [100] # set the number of episodes
        ...
        current_step = 0
        instruction, images_list = self.generate_input(observations[-1], current_step, envs.current_episodes()[0].episode_id)
        ...
        log_file = f"./nav_log/navigator_log_{envs.current_episodes()[0].episode_id}.log"
        ...
        if os.path.exists(actions_cache_path):
            with open(actions_cache_path, "w", encoding="utf-8") as file:
                json.dump({}, file, ensure_ascii=False, indent=2)
            actions_cache = {}
        else:
            actions_cache = {}
        
        navigator = ReThinkNav(self.device,config.LLM, config.API_KEY)
        nav_history = []
        ...
        error_number = 0
        min_step_length = 6
        start_episode = envs.current_episodes()[0].episode_id if envs.num_envs > 0 else None
        while envs.num_envs > 0 and len(stats_episodes) < episodes_to_eval:
            ...
            match_step_info = []
            match_step_vp = None
            agent_state = envs.call_at(0, "get_agent_info", {})
            ...
            max_step_length = 20
            nav_logger.info(f"Max step length: {max_step_length}")
            stop_flag = False
            ...
            nav_logger.info("=========== Get WayPointData =========")

            images_dict, radius_dict, distance_dict = self.construct_image_dicts(batch_distances[-1], batch_angles, images_list)
            nav_logger.info("batch_angles: %s", batch_angles)
            nav_logger.info("batch_distance: %s", batch_distances)
            nav_logger.info("radius_dict: %s", radius_dict)
            nav_logger.info("distance_dict: %s", distance_dict)
            ...
            position_file = f"cache_files/{dataset_name}/{current_episodes[0].episode_id}_position.json"
            os.makedirs(os.path.dirname(position_file), exist_ok=True)

            position_history = [] if current_step == 0 else (
                json.load(open(position_file)) if os.path.exists(position_file) else []
            )
            step_data = {
                "step": current_step,
            ...
            position_history.append(step_data)
            
            if not stop_flag:
                nav_logger.info("========== Estimate Completion Progress ==========")
                if current_step != 0:
                    nav_logger.info("========== Compute Relative Movements ==========")
                    movements = navigator.compute_relative_movements(nav_logger, position_history)
                estimation = navigator.estimate_completion(nav_logger, actions, landmarks, history_traj, observation, last_estimation, movements)
                ...
                if self.parse_done_or_not(estimation) and current_step < 6:
                    min_step_length = 6 if len(actions.splitlines()) <= 4 else 8


                if self.parse_done_or_not(estimation) and current_step >= min_step_length:
                    stop_flag = True
                    nav_logger.info("Navigation task successfully completed")
                    
                else:
                    nav_logger.info("========== Detect Loop ==========")
                    loop_flags, loop_info = navigator.detect_latest_loop_with_heading(nav_logger, position_history)
                    nav_logger.info(f"loop_info: {loop_info}")
                    nav_logger.info(f"loop_flags: {loop_flags}")
                    detect_flag = loop_flags

                    if not detect_flag:
                        nav_logger.info("========== Next Action Prediction ==========")
                        predictions, thoughts, break_flag, score, pred_vp = navigator.move_to_next_vp(nav_logger, current_step, instruction, actions, landmarks, history_traj, estimation, observation, observe_dict)
                    else:
                        nav_logger.info("========== Next Action Prediction in Loop ==========")
                        valid_viewpoints = set(map(str, observe_dict.keys()))
                        match_step_info, match_step_vp, current_step_id, current_step_score = navigator.compare_matched_visual_similarity(
                            nav_logger, nav_history, loop_info, position_history, clip_sim, current_episodes[0].episode_id, 
                            current_step, valid_viewpoints
                        )
                        predictions, thoughts, break_flag, score, pred_vp = navigator.move_to_next_vp_with_loop(nav_logger, current_step, instruction, actions, landmarks, history_traj, match_step_info, current_step_id, estimation, observation, observe_dict)
                    
                    position_history[-1]["score"] = score
                    position_history[-1]["pred_vp"] = pred_vp
                    with open(position_file, "w") as f:
                        json.dump(position_history, f, indent=2)
            ...
            try:
                if not stop_flag:
                    ...
                    if current_step == max_step_length:
                        dones[0] = True 
                    if stop_flag:
                        dones[0] = True
                    else:
                ...
                for i in range(envs.num_envs):
                    ...
                    if True: 
                        realtime_fname = os.path.join(
                        config.RESULTS_DIR,
                        f"realtime_stats_rank{self.local_rank}.jsonl"
                                        )
                    with open(realtime_fname, "a") as f:
                        f.write(json.dumps({
                    ...
                    instruction, images_list = self.generate_input_without_delete(observations[i],current_step, envs.current_episodes()[0].episode_id)
                    if config.use_pbar:
                        pbar.update()
                    else:
                        logger.info(
